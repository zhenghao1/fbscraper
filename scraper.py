...
def extract(page, numOfPost, infinite_scroll=False, scrape_comment=False):
    ...
    if scrape_comment:
        #first uncollapse ALL collapsed comments
        unCollapseCommentsButtonsXPath = '//div[contains(@class,"_666h")][@role=\'button\']'
        ...
        rankDropdowns = browser.find_elements(By.XPATH, "//div[contains(@class, 'abiwlrkh p8dawk7l')][@role='button']/span[contains(@class, 'lrazzd5p m9osqain')]")
        rankDropdowns = [rd for rd in rankDropdowns if rd.text != '']
        #rankDropdowns = browser.find_elements_by_class_name('_2pln') #select boxes who have rank dropdowns
        rankXPath = '//div[contains(@class, "l9j0dhe7")][@role="menu"]//div[contains(@class, "pfnyh3mw du4w35lb")][@role="menuitem"]//span[text()="All Comments"]'
        ...
        time.sleep(10)
        moreComments = browser.find_elements_by_xpath('//div[contains(@class,"gpro0wi8 m9osqain buofh1pr")][@role="button"]//span[contains(@class, "a3bd9o3v lrazzd5p m9osqain")]')
        viewMoreComments = [mc for mc in moreComments if 'view' in mc.text.lower()]
        
        print("Scrolling through to click on more comments")
        while len(viewMoreComments) != 0:
            for comment in viewMoreComments:
                action = webdriver.common.action_chains.ActionChains(browser)
                try:
